function Labeler(labels, colors, labeling_boxes) {
...
Labeler.prototype.show_entities = function() {
	...
	for (let i = 0; i < this.entities.length; i++) {
		tmp.push({
			...
			x: Math.floor(Math.max(0, this.entities[i].x / width * this.scale * IMAGE_WIDTH)),
	...
	$("#entities-data").text(JSON.stringify({ name: name, width: IMAGE_WIDTH, height: IMAGE_HEIGHT, entities: tmp }, null, "  ")); // отображаем JSON представление объектов
}
...
Labeler.prototype.end_labeling = function(select, entity_text) {
	...
	this.labeling_boxes.shift()

	if (this.labeling_boxes.length > 0)
...
// инициализация из выделенных объектов
Labeler.prototype.add_entity = function(entity) {
	// получаем размеры изображения
	let imageWidth = $(".labeler-image img").width()
	let imageHeight = $(".labeler-image img").height()
	...
	let new_entity = {
		label : entity.label,
		x : Math.floor(entity.x * imageWidth),
		y : Math.floor(entity.y * imageHeight),
		width : Math.floor(entity.width * imageWidth),
		height : Math.floor(entity.height * imageHeight)
	}

	this.currBox = $('<div class="label-box" draggable=false></div>') // создаём блок	
	this.currBox.appendTo(this.img) // добавляем блок к картинке и настраиваем его стили
	this.currBox.css({
		"outline": "2px solid " + this.get_color(entity.label),
		"position": "absolute",
		"top": entity.y * imageHeight * this.scale + "px",
		"left": entity.x * imageWidth * this.scale + "px",
		"width": entity.width * imageWidth * this.scale + "px",
		"height": entity.height * imageHeight * this.scale + "px",
		"background": this.get_color(entity.label, true)
	})
	
	$(document).scrollTop(entity.y * imageHeight * this.scale / 2)
...
Labeler.prototype.keydown = function(e) {
	...
	else if (e.key == " " && !this.isBlocked) {
		if (this.labeling_boxes.length > 0)
			this.add_entity(this.labeling_boxes[0])
		else
			alert("Tesseract больше ничего не нашёл :С")
